...
;(function($){
	$.contextMenu = {
		...
		prevMenu: null, // previously opened menu
		menuItems: {},
		noIdCount: 0,

		// Create the menu instance
		create: function(menu, root, opts) {
		...
		createMenuItem: function(label,obj) {
			...
			var $div = $('<div class="'+cmenu.itemClassName+' '+o.className+((o.disabled)?' '+cmenu.disabledItemClassName:''+' context-menu-item-'+o.id)+'" title="'+o.title+'"'+(typeof o.data != 'undefined' ? 'data-data="' + o.data + '"': '')+'></div>')
		...
		show: function(t,e) {
			if ($.contextMenu.prevMenu) $.contextMenu.prevMenu.hide();
		...
		hide: function() {
			if (this.doNotHideOnLeftClick) return;
			var cmenu=this;
			if (cmenu.shown) {
		...
		// bind menu to element
		bind: function(el, action) {
			var menu = this;
			this.action = typeof action == "function" ? action : function(){};
			el.each(function(){
				var $node = $(this);
				$node.on('contextmenu',function(e){menu.action($node, menu.menu);menu.show(this,e);return false;});
			});
		},
		...
		on: function(selector, action) {
			var menu = this;
			this.action = typeof action == "function" ? action : function(){};
			$(document).on('contextmenu', selector, function(e){menu.action($(this), menu.menu);menu.show(this,e);return false;});
		},

		// ShowElements
		showItems: function(filterItems) {
			if(filterItems instanceof Array)
			{
				for( var m in this.menuItems )
				{
					this.menuItems[m].hide();
				}
				for( var i = 0; i < filterItems; ++i )
				{
					if( this.menuItems[filterItems[i]] )
						this.menuItems[filterItems[i]].show();
				}
			}
			else
			{
	...
	$.fn.contextMenu = function(menu,options) {
		var cmenu = $.contextMenu.create(menu,this,options);
		return cmenu;
	};
})(jQuery);
